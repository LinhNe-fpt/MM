 ...
 * - Luồng **UPK** (`/upk`): admin, upk
 * - Luồng **RMA** (`/rma`): admin, rma
...
/** Luồng kho UPK (`/upk`) — admin hoặc vai trò upk */
export function duocVaoUpk(quyen: string): boolean {
  return quyen === "admin" || quyen === "upk";
}

/** Luồng kho RMA (`/rma`) — admin hoặc vai trò rma */
export function duocVaoRma(quyen: string): boolean {
  return quyen === "admin" || quyen === "rma";
}
